...
Self-hosted vector tiles for outdoor features (POIs, hiking routes) for the OpenFreeMap outdoor style. Uses [Planetiler](https://github.com/onthegomap/planetiler) to generate vector tiles from OSM data.
...
The outdoor build script (`scripts/build.mjs` in `styles/outdoor/`) uses local URLs when `POI_USE_LOCAL` / `OUTDOOR_ROUTE` are `true`:
